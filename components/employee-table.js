class EmployeeTable{
  ...
  updateEmployees(employees){
    ...
    if(employees && employees.length){
      ...
      for (let dataIndex = 0; dataIndex < employees.length; dataIndex++) {
        tbody.append(this.renderEmployeeRow(employees[dataIndex], this.deleteEmployee, this.editEmployee));
      }
    } else {
  ...
  renderEmployeeRow(data, deleteEmployee, editEmployee){
    ...
    updateButton.addEventListener("click", function () { 
      editEmployee(data.name, data.lastName, data.position, data.department_name, data.salary, data.id) 
    });

    deleteButton.classList.add("btn", "btn-outline-danger", "border", "border-0","fas", "fa-trash");
    deleteButton.addEventListener("click", function () { deleteEmployee(data.id) });
